from fastapi import APIRouter, UploadFile, HTTPException, Depends, File, status, Query
...
@router.post("/add")
def add_pdf(file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
